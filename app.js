(function () {
  ...
  function setOrigen(t) { const el = document.getElementById("origen"); if (el) el.textContent = t; }
  ...
  function pullSheets() {
    ...
    return fetch(url + "?token=" + encodeURIComponent(CFG.TOKEN || PASS))
      ...
      .then(function (json) {
        if (json && json.ok && json.data) { window.DATA = json.data; persistLocal(); setOrigen("Conectado a Google Sheets"); return true; }
        setOrigen("Sheets no respondió — usando copia local"); return false;
      })
      .catch(function () { setOrigen("Sin conexión a Sheets — copia local"); return false; });
  }
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (u === USER && document.getElementById("pass").value === PASS) { sessionStorage.setItem(KEY, "1"); showApp(); }
    else err.hidden = false;
  });
  document.getElementById("salir").addEventListener("click", function () { sessionStorage.removeItem(KEY); location.reload(); });
  function loadData() {
    if (window.DATA) return boot();
    const s = document.createElement("script"); s.src = "data.js"; s.onload = boot; document.body.appendChild(s);
  }
  function boot() {
    try { const saved = JSON.parse(localStorage.getItem(STORE) || ""); if (saved && saved.asuntos) window.DATA = saved; } catch (e) {}
    pullSheets().then(function () { start(); });
  }
  function nextAsunto() {
    const n = DATA.asuntos.reduce(function (m, a) { const x = parseInt(String(a.id).replace("CHV-", ""), 10); return isNaN(x) ? m : Math.max(m, x); }, 0);
  ...
  function nextTarea(aid) {
    const n = DATA.tareas.filter(function (t) { return t.asunto === aid; }).reduce(function (m, t) {
      const p = String(t.id).split("-"); const x = parseInt(p[p.length - 1], 10); return isNaN(x) ? m : Math.max(m, x);
    }, 0);
  ...
  function openModal(html) {
    ...
    const x = sheet.querySelector("[data-close]"); if (x) x.addEventListener("click", closeModal);
    modal.onclick = function (e) { if (e.target === modal) closeModal(); };
  }
  function opts(list, val) { return list.map(function (v) { return "<option" + (v === val ? " selected" : "") + ">" + v + "</option>"; }).join(""); }
  ...
  function asuOpts(val) {
    return DATA.asuntos.map(function (a) { return "<option value=\"" + a.id + "\"" + (a.id === val ? " selected" : "") + ">" + a.id + " · " + a.asunto + "</option>"; }).join("");
  }
  ...
  function formCas() {
    ...
    document.getElementById("fC").onsubmit = function (e) {
      e.preventDefault();
      sheet.querySelectorAll("input[name=nombre]").forEach(function (inp) { const i = Number(inp.getAttribute("data-i")); DATA.casilleros[i].nombre = inp.value.trim() || DATA.casilleros[i].nombre; DATA.casilleros[i].codigo = DATA.casilleros[i].nombre; });
      sheet.querySelectorAll("input[name=orden]").forEach(function (inp) { DATA.casilleros[Number(inp.getAttribute("data-i"))].orden = Number(inp.value || 1); });
      sheet.querySelectorAll("input[name=activo]").forEach(function (inp) { DATA.casilleros[Number(inp.getAttribute("data-i"))].activo = inp.checked; });
  ...
  function exportar() {
    const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob(["const DATA = " + JSON.stringify(DATA) + ";\n"], { type: "text/javascript" })); a.download = "data.js"; a.click();
  }
  function start() {
    ...
    function tareasDe(id) { return DATA.tareas.filter(function (t) { return t.asunto === id; }).sort(function (a, b) { return a.orden - b.orden; }); }
    function match(a) {
    ...
    function render() {
      ...
      $("stats").innerHTML = '<span class="chip">' + items.length + " asuntos</span><span class="chip">" + DATA.asuntos.filter(function (a) { return a.estado.indexOf("Abierto") === 0; }).length + " abiertos</span><span class="chip">" + DATA.tareas.filter(function (t) { return t.estado !== "Hecha"; }).length + " tareas pendientes</span>";
      list.innerHTML = "";
      items.forEach(function (a) {
        const ts = tareasDe(a.id); const el = document.createElement("article"); el.className = "asunto";
        el.innerHTML = '<button class="asunto-head" type="button"><div class="row1"><span class="id">' + a.id + '</span><span class="badge ' + a.estado + '">' + a.estado + "</span></div><div class="cas">" + a.casillero + "</div><h2 class="title">" + a.asunto + "</h2><div class="meta">" + (a.dueno || "") + " · " + ts.length + " tarea" + (ts.length === 1 ? "" : "s") + " · " + (a.proximo || "") + "</div></button><div class="tareas"><div class="mini"><button type="button" class="edA ghost">Editar asunto</button><button type="button" class="addT ghost">+ Tarea</button></div>" + (ts.map(function (t) { return '<div class="tarea ' + t.estado + '" data-tid="' + t.id + '"><div class="row1"><span class="tid">' + t.id + " · orden " + t.orden + '</span><span class="badge ' + t.estado + '">' + t.estado + "</span></div><div>" + t.titulo + "</div>" + (t.depende_de ? '<div class="cond">Depende de ' + t.depende_de + "</div>" : "") + (t.comentarios ? '<div class="cond">' + t.comentarios + "</div>" : "") + '<button type="button" class="edT ghost">Editar</button></div>'; }).join("") || '<div class="tarea">Sin tareas.</div>') + "</div>";
        ...
        el.querySelectorAll(".edT").forEach(function (btn) { btn.addEventListener("click", function (e) { e.stopPropagation(); formTarea(DATA.tareas.filter(function (x) { return x.id === btn.closest(".tarea").getAttribute("data-tid"); })[0]); }); });
        list.appendChild(el);
      });
    }
