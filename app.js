(function () {
  ...
  function nextTarea(aid) {
    const same = DATA.tareas.filter(function (t) { return t.asunto === aid; });
    const n = same.reduce(function (m, t) {
  ...
  function formAsunto(a) {
    ...
    openModal(
      "<h3>" + (DATA.asuntos.some(function (x) { return x.id === a.id; }) ? "Editar" : "Nuevo") + " asunto</h3>" +
      "<form id=\"fA\">" +
      "<label>Código<input name=\"id\" value=\"" + a.id + "\" readonly></label>" +
      "<label>Casillero<select name=\"casillero\">" + casOpts(a.casillero) + "</select></label>" +
      "<label>Asunto<input name=\"asunto\" required value=\"" + (a.asunto || "") + "\"></label>" +
      "<label>Estado<select name=\"estado\">" + opts(EST_A, a.estado) + "</select></label>" +
      "<label>Dueño<input name=\"dueno\" value=\"" + (a.dueno || "") + "\"></label>" +
      "<label>Próximo<input name=\"proximo\" value=\"" + (a.proximo || "") + "\"></label>" +
      "<label>Notas<textarea name=\"notas\">" + (a.notas || "") + "</textarea></label>" +
      "<div class=\"rowbtns\"><button type=\"submit\">Guardar</button><button type=\"button\" data-close class=\"ghost\">Cancelar</button></div></form>"
    );
    document.getElementById("fA").onsubmit = function (e) {
      e.preventDefault();
      const fd = new FormData(e.target);
      const row = {
        id: fd.get("id"), casillero: fd.get("casillero"), asunto: fd.get("asunto"),
        estado: fd.get("estado"), dueno: fd.get("dueno"), proximo: fd.get("proximo"),
        notas: fd.get("notas"), carpeta: a.carpeta || "", link: a.link || "", cuenta: a.cuenta || "oficina"
      };
  ...
  function formTarea(pre) {
    ...
    openModal(
      ...
      "<form id=\"fT\">" +
      "<label>Asunto<select name=\"asunto\">" + asuOpts(t.asunto) + "</select></label>" +
      "<label>Título<input name=\"titulo\" required value=\"" + (t.titulo || "") + "\"></label>" +
      "<label>Estado<select name=\"estado\">" + opts(EST_T, t.estado) + "</select></label>" +
      "<label>Orden<input name=\"orden\" type=\"number\" min=\"1\" value=\"" + (t.orden || 1) + "\"></label>" +
      "<label>Depende de<input name=\"depende_de\" value=\"" + (t.depende_de || "") + "\" placeholder=\"CHV-001-01\"></label>" +
      "<label>Comentarios<textarea name=\"comentarios\">" + (t.comentarios || "") + "</textarea></label>" +
      "<div class=\"rowbtns\"><button type=\"submit\">Guardar</button><button type=\"button\" data-close class=\"ghost\">Cancelar</button></div></form>"
    );
    document.getElementById("fT").onsubmit = function (e) {
      ...
      const row = {
        id: t.id || nextTarea(asunto),
        asunto: asunto,
        orden: Number(fd.get("orden") || 1),
        titulo: fd.get("titulo"),
        estado: fd.get("estado"),
        depende_de: fd.get("depende_de"),
  ...
  function formCas() {
    openModal(
      "<h3>Casilleros</h3>" +
      "<form id=\"fC\">" +
      DATA.casilleros.sort(function (a, b) { return a.orden - b.orden; }).map(function (c, i) {
        return "<div class=\"casrow\"><input name=\"nombre\" data-i=\"" + i + "\" value=\"" + c.nombre + "\">" +
          "<input name=\"orden\" type=\"number\" data-i=\"" + i + "\" value=\"" + c.orden + "\" style=\"width:70px\">" +
          "<label class=\"chk\"><input type=\"checkbox\" name=\"activo\" data-i=\"" + i + "\"" + (c.activo ? " checked" : "") + "> activo</label></div>";
      }).join("") +
      "<label>Nuevo casillero<input name=\"nuevo\" placeholder=\"Ej. 4 CONSULTORÍA\"></label>" +
      "<div class=\"rowbtns\"><button type=\"submit\">Guardar</button><button type=\"button\" data-close class=\"ghost\">Cancelar</button></div></form>"
    );
    document.getElementById("fC").onsubmit = function (e) {
      e.preventDefault();
      const names = sheet.querySelectorAll("input[name=nombre]");
  ...
  function exportar() {
    const blob = new Blob(["const DATA = " + JSON.stringify(DATA) + ";\n"], { type: "text/javascript" });
  ...
  function start() {
    ...
    function match(a) {
      ...
      const blob = [a.id, a.asunto, a.casillero, a.proximo].concat(ts.map(function (t) { return t.id + " " + t.titulo; })).join(" ").toLowerCase();
    ...
    function render() {
      ...
      const open = DATA.asuntos.filter(function (a) { return a.estado.indexOf("Abierto") === 0; }).length;
      ...
      items.forEach(function (a) {
        ...
        el.innerHTML =
          ...
          "</button>" +
          '<div class="tareas">' +
          '<div class="mini"><button type="button" class="edA ghost">Editar asunto</button>' +
          '<button type="button" class="addT ghost">+ Tarea</button></div>' +
          (ts.map(function (t) {
            return '<div class="tarea ' + t.estado + '" data-tid="' + t.id + '">' +
        ...
        el.querySelectorAll(".edT").forEach(function (btn) {
          btn.addEventListener("click", function (e) {
            e.stopPropagation();
            const id = btn.closest(".tarea").getAttribute("data-tid");
            formTarea(DATA.tareas.filter(function (x) { return x.id === id; })[0]);
          });
        });
        list.appendChild(el);
      });
