  ...
  function start() {
    ...
    function render() {
      ...
      items.forEach(function (a) {
        ...
        el.className = "asunto";
        el.innerHTML = '<div class="asunto-head"><div class="row1"><span class="id">' + a.id + '</span><div class="asunto-tools"><span class="badge">' + a.estado + '</span><button type="button" class="icon-btn edA" title="Editar asunto" aria-label="Editar asunto">✎</button><button type="button" class="icon-btn addT" title="Agregar tarea" aria-label="Agregar tarea">+</button></div></div><div class="cas">' + (a.casillero || "") + '</div><h2 class="title">' + a.asunto + '</h2><div class="meta">' + (a.dueno || "") + " · " + ts.length + " tarea" + (ts.length === 1 ? "" : "s") + " · " + (a.proximo || "") + "</div></div><div class=\"tareas\">" +
          (ts.map(function (t) {
